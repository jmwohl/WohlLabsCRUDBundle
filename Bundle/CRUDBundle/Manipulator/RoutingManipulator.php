...
namespace WohlLabs\Bundle\CRUDBundle\Manipulator;
...
class RoutingManipulator extends Manipulator
{
    ...
    public function addResource($bundle, $format, $prefix = '/', $path = 'routing')
    {
        ...
        if (file_exists($this->file)) {
			...
			// Append the prefix, if it exists
			$route_name = $bundle.('/' !== $prefix ? '_'.str_replace('/', '_', substr($prefix, 1)) : '');
            if (false !== strpos($current, $route_name)) {
                throw new \RuntimeException(sprintf('Route "%s" is already imported.', $route_name));
            }
